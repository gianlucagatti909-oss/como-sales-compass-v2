...
import { Search, ChevronRight, MessageSquareText, CalendarDays, X, FilterX } from "lucide-react";
...
export default function TPListPage({ records, hasGiacenza }: Props) {
  ...
  const rappresentanti = useMemo(
    () => [...new Set(records.map(r => r.rappresentante).filter(Boolean))].sort(),
    [records]
  );
  const zone = useMemo(
    () => [...new Set(records.map(r => r.tp_zona).filter(Boolean))].sort(),
    [records]
  );
  const tipi = useMemo(
    () => [...new Set(records.map(r => r.tp_tipo).filter(Boolean))].sort(),
    [records]
  );
  ...
  const filtered = useMemo(() => records.filter(r => {
    ...
    if (hasGiacenza && filterCat !== "all" && (r.categoria === null || r.categoria !== filterCat)) return false;
    return true;
  }).sort((a, b) => b.venduto_euro - a.venduto_euro), [records, search, filterRapp, filterZona, filterTipo, filterCat, hasGiacenza]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3">
        ...
        {(search || filterRapp !== "all" || filterZona !== "all" || filterTipo !== "all" || filterCat !== "all") && (
          <button
            onClick={() => { setSearch(""); setFilterRapp("all"); setFilterZona("all"); setFilterTipo("all"); setFilterCat("all"); }}
            className="inline-flex items-center gap-1.5 px-3 py-2 rounded-md border border-input text-sm text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors"
            title="Azzera tutti i filtri"
          >
            <FilterX className="w-4 h-4" /> Azzera filtri
          </button>
        )}
      </div>
